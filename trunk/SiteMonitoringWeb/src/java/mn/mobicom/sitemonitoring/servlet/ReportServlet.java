...
public class ReportServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Integer first = null;
            try {
                first = Integer.parseInt(request.getParameter("_first"));
            } catch (Exception ex) {
            }
            Integer max = null;
            try {
                max = Integer.parseInt(request.getParameter("_max"));
            } catch (Exception ex) {
            }
            if (name != null) {
                Map<String, String[]> map = request.getParameterMap();
                Map<String, Object> parms = ParmMapDeserializer.deserialize(map);
                System.out.println(parms);
                if (callback != null) {
                    out.write(callback + "(");
                }
                out.write(JsonTool.toJson(eventFacade.executeNamedQuery(name, parms, max, first)));
                if (callback != null) {
                    out.write(")");
                }
            } else if (request.getParameter("_custom") != null) {
            }
        } finally {
            out.close();
        }
